...
with load_workflow:
    ...
    curl_csv_green_job= BashOperator(
        task_id='download_csv_file_url',
        bash_command=f'curl -sSL {url_template} > {output_file_template}'
    )

    green_to_gcs_job = PythonOperator(
        task_id="csv_to_gcs_job",
        python_callable=upload_to_gcs,
        op_kwargs={
            "bucket": BUCKET,
            "object_name": f"raw_csv/{csv_file}",
...
curl_csv_green_job >> green_to_gcs_job >> cleanup_job